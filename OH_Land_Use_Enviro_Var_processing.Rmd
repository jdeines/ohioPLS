...
lcNmds$nmdsAll <- nmdsMin[,1]
lcNmdsOut <- merge(lcNmds,nmdsMin27, by.x='Polygon', by.y='ID',all=T)

# visualize nmds output
county <- readOGR("GISlayers","OHCounty", verbose=F)
countyTest <- merge(county, lcNmdsOut[,c(1,14:15)], by.x='ID',by.y = 'Polygon')
spplot(countyTest,c('nmdsAll','nmds27'), col.regions=colorRampPalette(c('forestgreen','gray88', 'darkmagenta'))(20))

write.csv(lcNmdsOut,"R Output/OHCountyLandCover_aggregated_nmds.csv",row.names=F)
```
