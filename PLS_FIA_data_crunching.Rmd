...
# Identify which polygon (in "county") each point in corners falls within using an overlay and store as new column
...
comp<-t(sapply(as.numeric(levels(factor(corners$countyID))),function(j)  {
	...
	c(county=j,sapply(species, function(i)sum(c(J$SP1==i,J$SP2==i,J$SP3==i,J$SP4==i),na.rm=TRUE))) 
}))

# combine typo columns
comp.df <- as.data.frame(comp)
comp.df <- rename(comp.df, c('sugar maple' = "sugarmaple", `sugar mpale` = "sugarmpale"))
comp.df$poplar <- comp.df$poplar + comp.df$Poplar
comp.df$sugarmaple <- comp.df$sugarmaple + comp.df$sugarmpale
comp.df <- rename(comp.df, c('sugarmaple' = "sugar maple"))
comp.df <- subset(comp.df, select=-c(Poplar,sugarmpale))

#add a column that is the sum of the trees for each county (sum row excluding county column)
comp.df <-cbind(comp.df,total=rowSums(comp.df[,-1]))
...
```
```{r PLStoCountyRaw_out, eval=FALSE}
# write output to CSV for intermediate starting point
write.csv(comp.df2,"R output/OHCountyCompRaw_PLS.csv", row.names=F)
```

This Ohio dataset includes `r numDataPoints` corner data locations and `r numTrees` trees recorded. There were `r length(species)-2` names recorded in the PLS dataset.  

**note:** retrieving the names directly from the data (as done above) discovered 2 species missed in the manual creation in past versions (persimmon and swamp ash), as well as two entry typos missed in cleaning ('sugar mpale' and 'Poplar').  I added swamp ash to the 'ash' category, and persimmon to 'other'.

**note:** This version omits writing out a shapefile with the raw county counts as it's probably not helpful


Data Cleaning and Aggregation: PLS
-----------------------------------
This aggregates the `r length(species)-2` common names into 15 taxonomic groups. Next, it excludes counties with low sample numbers from analysis (because the counties don't directly align with the township borders, there are a few counties with a small number of points that belong to townships mostly within neighboring counties).  

```{r cleanPLS, fig.height=5, fig.width=5}
setwd('C:/Users/deinesji/Dropbox/Dan_Williams/CleanCodeAndData')

# read in raw CSV 
raw.comp <- read.csv("R output/OHCountyCompRaw_PLS.csv")

#----PLS taxon groupings----# 
raw.comp2 <- cbind(raw.comp[,c('county','CountyName')],
	"ash"=rowSums(raw.comp[,c("ash", "black.ash", "blue.ash", 
                             "hoop.ash", "swamp.ash", "white.ash")]),
	"basswood"=rowSums(raw.comp[,c("basswood","linden")]),
	...
	"other"=rowSums(raw.comp[,c("red.bud", "chestnut", "poplar", "hemlock", 
              "birch", "aspen", "cottonwood", 'persimmon', "trembling.aspen", 
              "gum", "black.gum", "dogwood", "pepperidge", "mulberry", "ironwood",
              "serviceberry", "hornbeam", "coffeetree", "cedar", "locust", "black.locust",
              "buckeye","sassafras", "willow", "sourwood")]),
...
pls.comppct <- cbind(countyID = pls.comp$county, countyName = pls.comp$CountyName, pls.comp[,3:17]/pls.comp$total, 
  total = pls.comp$total)
```
```{r cleanPLS_out, eval=FALSE}
# write out data
...
pls.comp1 <- read.csv("R output/OHCompClean_PLS.csv")

# check PLS and FIA counties match
sum(sort(pls.comp1$county) != sort(FIA.spec1$id))

#----Combine Species Classes----#
FIA.spec2 <-cbind(FIA.spec1[,c("id","County")],
	"ash"=rowSums(FIA.spec1[,c("ash", "white.ash", "black.ash", "green.ash", "blue.ash")]),
	"basswood"=FIA.spec1$american.basswood,
	"beech"=FIA.spec1$american.beech,
	"cherry"=rowSums(FIA.spec1[,c("black.cherry","pin.cherry","chokecherry","sweet.cherry")]),
	"elm"=rowSums(FIA.spec1[,c("american.elm", "siberian.elm", "slippery.elm","rock.elm")]),
	...
	"maple"=rowSums(FIA.spec1[,c("black.maple", "red.maple", "silver.maple", "sugar.maple", 
                  "norway.maple", "boxelder")]),
	...
	"pine"=rowSums(FIA.spec1[,c("eastern.white.pine", "scotch.pine", "loblolly.pine", 
                "virginia.pine", "austrian.pine", "red.pine", "pitch.pine", "shortleaf.pine")]),
	"sycamore"=FIA.spec1$american.sycamore,
	"tulip"=FIA.spec1$yellow.poplar,
	"walnut"=rowSums(FIA.spec1[,c("black.walnut","butternut")]),
	"other"=rowSums(FIA.spec1[,c("chestnut","eastern.red.cedar","norway.spruce","blue.spruce",
                  "eastern.hemlock","unknown.dead.conifer","ohio.buckeye","yellow.buckeye",
                  "ailanthus","european.alder","yellow.birch","sweet.birch","river.birch",
                  "northern.catalpa","eastern.redbud","hawthorn","common.persimmon",
                  "kentucky.coffeetree","sweetgum","osage.orange","cucumbertree","apple",
                  "white.mulberry","blackgum","eastern.hophornbeam","sourwood",
                  "paulownia..empress.tree.","eastern.cottonwood","bigtooth.aspen",
                  "quaking.aspen","black.locust","black.willow","sassafras",
                  "unknown.dead.hardwood","other.or.unknown.live.tree")]),
...
write.csv(centroids, "R output/OH_county_centroids_projected.csv")
...
col <- c("#377EB8", "#984EA3", "#4DAF4A", "#E41A1C", "cyan2", "green", "#F781BF", "#0C2C84", "#FFFF33", "#FF7F00", "#A65628", "olivedrab1", "#999999", "#E7298A", "white")
# legend names (corresponds with color order)
species <- c("ash", "basswood", "beech", "cherry", "elm", "hackberry", "hickory", "honey locust", "maple", "oak", "pine", "sycamore", "tulip", "walnut", "other")

# turn on to write file to disk
png("exportedFigs/PieCharts_margTest.png", type = "cairo", units = "in", width = 7, height = 5, res = 300)
...
#png("exportedFigs/SpeciesChangeWorking.png", type = "cairo", units = "in", width = 7, height = 5, res = 300)


## LATTICE PLOTS
colchange <- c("blue4","blue3","blue","dodgerblue","cyan3","cyan","lightgreen",
  "yellow","gold","orange","darkorange2","orangered2","darkred")
spplot(change3[,14:28],col.regions=colchange, at=seq(-.55,.55,(.55+.55)/13),
       as.table=T, colorkey=list(labels=list(cex=1.2)))
#grid.text("Percent Difference", x=unit(0.97, "npc"), y=unit(0.50, "npc"), rot=90)
