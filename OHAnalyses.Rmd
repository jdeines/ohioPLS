
```{r options, include=FALSE}
opts_chunk$set(cache = TRUE)
```

Ohio Analyses
========================================================

Jillian Deines  
March 2014

This script analyses correlations among Ohio vegetation (historic and modern) and covariates (land use and environmental variables).

Analyses aim to describe presettlement vegetation in Ohio and examine relationships among
modern vegetation (using FIA data), historic vegetation, land use, and environmental variables.

#### Depends:
This script relies on output from the following scripts
* PLS_FIA_data_crunching.Rmd
* OH_Land_Use_Enviro_Var_processing.Rmd

#### Inputs:
* OHcountyLandCover.csv
* OH_EnviroVarCountySummary.csv
* OH_county_centroids_latlong.csv 
* OHCompCleanPLS_percent.csv
* OHCompCleanFIA_percent.csv

#### Crunching:
* Aggregate land use classes
* Basic correlation matrices between county composition (FIA/PLS) and 1) environmental variables and 2) land use
* Mantel tests: aggregated environmental and land use dissimilarity matrices

#### Outputs:
* 
* 

Working directory and packages
------------------------------
This assumes you have the file 'CleanCodeAndData' set as your working directory

```{r setup,message=FALSE}
setwd('C:/Users/deinesji/Dropbox/Dan_Williams/CleanCodeAndData')
library(ecodist)
library(corrplot)
library(RColorBrewer)
```


Correlations
--------------------------
PLS, FIA, environmental variables, and land use summaries are analyzed for correlations.  
* Proportional species composition data is arc sine square root transformed

**Note:** In previous versions of this script prior to Jill's pre-publication cleaning effort, there was a typo in the land use/composition correlations where the correlation between land use and PLS composition was stored as the variable 'fia.land', and vice versa. **Thus the patterns between land use and composition previously described in Dan's thesis version are reversed**. The number of the correlations differs due to data cleaning, but the pattern of strong/weak and positive/negative correlations is reversed.

```{r Correlations}
setwd('C:/Users/deinesji/Dropbox/Dan_Williams/CleanCodeAndData')

# read in data
pls <- read.csv("R output/OHCompCleanPLS_percent.csv")
fia <- read.csv("R Output/OHCompCleanFIA_percent.csv")
envar <- read.csv("R output/OH_EnviroVarCountySummary.csv")
landuse <- read.csv("R Output/OHCountyLandCover_aggregated_nmds.csv")
centroids <- read.csv("R output/OH_county_centroids_latlong.csv") 

# subset enviro and land use to 27 data counties, add coords
envar2   <- merge(centroids, envar, by.x = "ID", by.y = "countyID", all.y = FALSE)
landuse2 <- merge(centroids, landuse, by.x = "ID", by.y = "Polygon", all.y = FALSE)

# arcsine squareroot transform proportional data (land use, pls, fia)
pls[,3:17]     <- asin(sqrt(pls[,3:17]))		
fia[,3:17]     <- asin(sqrt(fia[,3:17]))	
landuse2[,5:14] <- asin(sqrt(landuse2[,5:14])) 

# correlations: composition vs enviro, landuse 
plsEnvCor <-cor(envar2[,5:11], pls[,3:17])	
fiaEnvCor <-cor(envar2[,5:11], fia[,3:17])	

#plsLandCor <- cor(landuse2[,5:14], pls[,3:17])	
fiaLandCor <- cor(landuse2[,5:14], fia[,3:17])	
```

### Correlation Plots
Needed: Nicer axis labels, significance indication?
```{r CorrPlots, echo=FALSE}
#pls/environmental variables
corrplot(plsEnvCor, method="color", addCoef.col="black", title="PLS-Enviroment", tl.col="black", 
  col=colorRampPalette(c("darkmagenta", "white", "darkgreen"))(200))
#fia/enviro
corrplot(fiaEnvCor, method="color", addCoef.col="black", title="FIA-Environment", tl.col="black", 
  col=colorRampPalette(c("darkmagenta", "white", "darkgreen"))(200))

# #pls/land use
# corrplot(plsLandCor, method="color", addCoef.col="black", title="PLS-Land Use", tl.col="black", 
#   col=colorRampPalette(c("darkmagenta", "white", "darkorange2"))(200))
#fia/land use
corrplot(fiaLandCor, method="color", addCoef.col="black", title="FIA-Land Use", tl.col="black", col=colorRampPalette(c("darkmagenta", "white", "darkorange2"))(200))
```


Mantel Tests
---------------------------

Other thoughts for Alex's notes: 
* This is areal/zonal data (vs points or continuous)
* Proximity: currently use county centroids in some fashion...but whaat about common boundary?

 Mantel tests? 1000 permutations seems standard??
### Load Data and Get Distance Matrices

```{r Mantels_loadData}
setwd('C:/Users/deinesji/Dropbox/Dan_Williams/CleanCodeAndData')

pls <- read.csv("R output/OHCompCleanPLS_percent.csv")
fia <- read.csv("R Output/OHCompCleanFIA_percent.csv")
envar <- read.csv("R output/OH_EnviroVarCountySummary.csv")
landuse <- read.csv("R Output/OHCountyLandCover_aggregated_nmds.csv")
centroids <- read.csv("R output/OH_county_centroids_latlong.csv") 

# subset enviro and land use to 27 data counties, add coords
envar2   <- merge(centroids, envar, by.x = "ID", by.y = "countyID", all.y = FALSE)
landuse2 <- merge(centroids, landuse, by.x = "ID", by.y = "Polygon", all.y = FALSE)

# arcsine squareroot transform proportional data (land use, pls, fia)
pls[,3:17]     <- asin(sqrt(pls[,3:17]))  	
fia[,3:17]     <- asin(sqrt(fia[,3:17]))	
landuse2[,5:14] <- asin(sqrt(landuse2[,5:14])) 
```

### Big Picture: Enviroment/climate vs Land Use
Reference: Goslee & Urban 2007  
Dissimilarity matrices for composition used the Bray-Curtis distance methoc, as recommended by Goslee & Urban.

First, I test for spatial autocorrelation in composition data; since this is positive, I use partial Mantel tests to test relationships of composition and land use/enviro

```{r Mantel1}
# composition distance matrices
FIAdist <- ecodist::distance(fia[,3:17], method= "bray-curtis") 
PLSdist <- ecodist::distance(pls[,3:17], method= "bray-curtis")

# ----- spatial autocoreelation -----------
spatialDist<- ecodist::distance(centroids[,c('X','Y')], method = "euclidean")

PLS_space <- mantel(PLSdist ~ spatialDist)
FIA_space <- mantel(FIAdist ~ spatialDist)
# yes, data is spatially correlated

# ----- total enviro and land use ------------
luDist <- ecodist::distance(landuse2[,5:14], method = "euclidean")
envirDist <- ecodist::distance(envar2[,5:11], method = "euclidean")

# species vs environment, given effects of space
PLS_env_space <- mantel(PLSdist ~ envirDist + spatialDist)
FIA_env_space <- mantel(FIAdist ~ envirDist + spatialDist)

PLS_lu_space <- mantel(PLSdist ~ luDist + spatialDist)
FIA_lu_space <- mantel(FIAdist ~ luDist + spatialDist)
```

The Big Picture results indicate that variation in PLS and environmental dissimilarities matrices are significantly correlated. FIA and environment are not significantly correlated, but FIA is significantly correlated with modern land use. 

### Individual Environmental Variables
For both PLS and FIA data. For Dan's project, we ran the partial Mantel test with only space as the additional variables; the Git commit of this script on May 12, 2014 replicates Dan's results (except Dan's report does not show significance in FIA soil water and annual ppt, which mine do - we get the same mantel r, so I think Dan failed to label the significant tests for FIA/environment), which are also reported in his final paper.

Here, I fully cross the partial Mantels, adding all other variables tested in addition to space. This changes the results as follows: 1) previously non-significant variables have much lower mantel r's and remain insignificant; and 2) previously significant variables are still significant (annptt p value lowered...), and their mantel r's are similar but a bit lower. For FIA data, however, these changes had a larger effect: Annual precip became insignificant, and minimum temperature became significant. Weird.


```{r Mantel_enviro}
# distance matrices for individual environmental variables examined
soilphDist  <- ecodist::distance(envar2[,'soilph'], method = "euclidean")
slopeDist   <- ecodist::distance(envar2[,'slope'], method = "euclidean")
soilh20Dist <- ecodist::distance(envar2[,'soilh2o'], method = "euclidean")
tempannDist <- ecodist::distance(envar2[,'tempann'], method = "euclidean") 
pptannDist  <- ecodist::distance(envar2[,'pptann'], method = "euclidean")
tminjanDist <- ecodist::distance(envar2[,'tminjan'], method = "euclidean")
tmaxjulDist <- ecodist::distance(envar2[,'tmaxjul'], method = "euclidean")

# PLS tests
PLSsoil    <- mantel(PLSdist ~ soilphDist + spatialDist + slopeDist + soilh20Dist + tempannDist + pptannDist + tminjanDist + tmaxjulDist )
PLSslope   <- mantel(PLSdist ~ slopeDist + spatialDist + soilphDist + soilh20Dist + tempannDist + pptannDist + tminjanDist + tmaxjulDist)
PLSsoilH2O <- mantel(PLSdist ~ soilh20Dist + spatialDist + soilphDist + slopeDist + tempannDist + pptannDist + tminjanDist + tmaxjulDist)
PLStempann <- mantel(PLSdist ~ tempannDist + spatialDist + soilphDist + slopeDist + soilh20Dist + pptannDist + tminjanDist + tmaxjulDist)
PLSpptann  <- mantel(PLSdist ~ pptannDist + spatialDist + soilphDist + slopeDist + soilh20Dist + tempannDist + tminjanDist + tmaxjulDist )
PLStminjan <- mantel(PLSdist ~ tminjanDist + spatialDist + soilphDist + slopeDist + soilh20Dist + tempannDist + pptannDist + tmaxjulDist)
PLStmaxjul <- mantel(PLSdist ~ tmaxjulDist + spatialDist + soilphDist + slopeDist + soilh20Dist + tempannDist + pptannDist + tminjanDist)

# FIA tests
FIAsoil    <- mantel(FIAdist ~ soilphDist + spatialDist + slopeDist + soilh20Dist + tempannDist + pptannDist + tminjanDist + tmaxjulDist )
FIAslope   <- mantel(FIAdist ~ slopeDist + spatialDist + soilphDist + soilh20Dist + tempannDist + pptannDist + tminjanDist + tmaxjulDist)
FIAsoilH2O <- mantel(FIAdist ~ soilh20Dist + spatialDist + soilphDist + slopeDist + tempannDist + pptannDist + tminjanDist + tmaxjulDist)
FIAtempann <- mantel(FIAdist ~ tempannDist + spatialDist + soilphDist + slopeDist + soilh20Dist + pptannDist + tminjanDist + tmaxjulDist)
FIApptann  <- mantel(FIAdist ~ pptannDist + spatialDist + soilphDist + slopeDist + soilh20Dist + tempannDist + tminjanDist + tmaxjulDist)
FIAtminjan <- mantel(FIAdist ~ tminjanDist + spatialDist + soilphDist + slopeDist + soilh20Dist + tempannDist + pptannDist + tmaxjulDist)
FIAtmaxjul <- mantel(FIAdist ~ tmaxjulDist + spatialDist + soilphDist + slopeDist + soilh20Dist + tempannDist + pptannDist + tminjanDist)
```

### Individual Land Use Variables
For FIA data; fully adding variables to partial Mantel test resulted in forest land use no longer being significant.


```{r Mantel_lu}
# distance matrices for individual land use variables
Openh2odist <- distance(landuse2[,'openWater'], method = "euclidean")
DevOpendist <- distance(landuse2[,'developedOpen'], method = "euclidean")
Devdist     <- distance(landuse2[,'developed'], method = "euclidean")
Barrendist  <- distance(landuse2[,'barren'], method = "euclidean")
Forestdist  <- distance(landuse2[,'forest'], method = "euclidean")
Shrubdist   <- distance(landuse2[,'shrub'], method = "euclidean")
Grassdist   <- distance(landuse2[,'grassland'], method = "euclidean")
Pasturedist <- distance(landuse2[,'pasture'], method = "euclidean")
Cropsdist   <- distance(landuse2[,'crops'], method = "euclidean")
Wetlanddist <- distance(landuse2[,'wetlands'], method = "euclidean")

#partial mantel test with FIA data: land use
Openh2odist + DevOpendist + Devdist + Barrendist + Forestdist + Shrubdist + Grassdist + Pasturedist + Cropsdist + Wetlanddist

FIAopenH2O <- mantel(FIAdist ~ Openh2odist + spatialDist + DevOpendist + Devdist + Barrendist + Forestdist + Shrubdist + Grassdist + Pasturedist + Cropsdist + Wetlanddist)
FIAdevOpen <- mantel(FIAdist ~ DevOpendist + spatialDist + Openh2odist + Devdist + Barrendist + Forestdist + Shrubdist + Grassdist + Pasturedist + Cropsdist + Wetlanddist)
FIAdev     <- mantel(FIAdist ~ Devdist + spatialDist + Openh2odist + DevOpendist + Barrendist + Forestdist + Shrubdist + Grassdist + Pasturedist + Cropsdist + Wetlanddist)
FIAbarren  <- mantel(FIAdist ~ Barrendist + spatialDist + Openh2odist + DevOpendist + Devdist + Forestdist + Shrubdist + Grassdist + Pasturedist + Cropsdist + Wetlanddist)
FIAforest  <- mantel(FIAdist ~ Forestdist + spatialDist + Openh2odist + DevOpendist + Devdist + Barrendist + Shrubdist + Grassdist + Pasturedist + Cropsdist + Wetlanddist)
FIAshrub   <- mantel(FIAdist ~ Shrubdist + spatialDist + Openh2odist + DevOpendist + Devdist + Barrendist + Forestdist + Grassdist + Pasturedist + Cropsdist + Wetlanddist)
FIAgrass   <- mantel(FIAdist ~ Grassdist + spatialDist + Openh2odist + DevOpendist + Devdist + Barrendist + Forestdist + Shrubdist + Pasturedist + Cropsdist + Wetlanddist)
FIApasture <- mantel(FIAdist ~ Pasturedist + spatialDist + Openh2odist + DevOpendist + Devdist + Barrendist + Forestdist + Shrubdist + Grassdist + Cropsdist + Wetlanddist)
FIAcrops   <- mantel(FIAdist ~ Cropsdist + spatialDist + Openh2odist + DevOpendist + Devdist + Barrendist + Forestdist + Shrubdist + Grassdist + Pasturedist + Wetlanddist)
FIAwetland <- mantel(FIAdist ~ Wetlanddist + spatialDist + Openh2odist + DevOpendist + Devdist + Barrendist + Forestdist + Shrubdist + Grassdist + Pasturedist + Cropsdist)
```

### Generate Mantel Tests Output
```{r mantel_out}

```











Composition NMDS
----------------------------
Code raw from previous version - not yet updated

```{r}
## NMDS for PLS Data##

#run NMDS
PLS.NMDS<-nmds(PLS.dist, maxdim=2)

#view results for each iteration
PLS.NMDS$stress #1-r^2
PLS.NMDS$r2 #percent variation each configuration explains

# get the NMDS scores for each county for the configuration which had the minimum
#stress values, aka captures the most variance in the data, or has the
# maximum r2 value

PLS.nmds.min<- nmds.min(PLS.NMDS)

#Plot NMDS min stress result
plot(PLS.nmds.min)

#Fit vectors for the main variables to the NMDS configuration
PLS.nmds.vf<- vf(PLS.nmds.min, pls[,4:18], nperm=10) #why 10?
plot(PLS.nmds.vf, col="blue")

## NMDS for FIA Data ##

#run NMDS
FIA.NMDS<-nmds(FIA.dist, maxdim=2)

#view results for each iteration
FIA.NMDS$stress #1-r^2
FIA.NMDS$r2 #percent variation each configuration explains

# get the NMDS scores for each county for the configuration which had the minimum
#stress values, aka captures the most variance in the data, or has the
# maximum r2 value

FIA.nmds.min<- nmds.min(FIA.NMDS)

#Plot NMDS min stress result
plot(FIA.nmds.min)

#Fit vectors for the main variables to the NMDS configuration
FIA.nmds.vf<- vf(PLS.nmds.min, pls[,4:18], nperm=10) #why 10?
plot(FIA.nmds.vf, col="blue")
```

