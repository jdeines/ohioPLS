...
# output dataframe
MantelResults <- data.frame(variable = rep('tbd',30), dataset = NA, mantelr = NA, pval = NA)
...
pls$color <- apply(as.array(pls$region),1,function (x) {
  if (x == 1) out <- 'darkorange2'
  else out <- 'darkmagenta'
  return(out) })
...
par(mar=c(5,4,2,1))
plot(PLS.nmds.min, main='Settlement-Era', col=pls$color, pch=16, xlim=xrange, ylim=yrange)
plot(PLS.nmds.vf)
legend('bottomleft', legend=paste0('r^2 = ',round(max(PLSnmds$r2),2)), bty='n')  

par(mar=c(5,4,2,1))
plot(FIA.nmds.min, main='Modern', col=pls$color, pch=16, xlim=xrange, ylim=yrange)
plot(FIA.nmds.vf)
legend('bottomleft', legend=paste0('r^2 = ',round(max(FIAnmds$r2),2)), bty='n') 

# empty plot over full device
par(fig=c(0, 1, 0, 1), oma=c(0, 0, 0, 0), mar=c(0, 0, 0, 0), new=TRUE)
plot(0, 0, type='n', bty='n', xaxt='n', yaxt='n')
# add legend (for example)
legend("bottom", legend=c('southeast counties','northwest counties'), fill = c('darkmagenta','darkorange2'), bty='n')
...
County Composition Stats
-----------------------------
Quantitative descriptions of composition and changes
